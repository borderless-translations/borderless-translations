...
function* toggleAvailabilityAdmin(action) {
    console.log("toggleAvailabilityAdmin")
    try {
        ...
        yield axios.put(`/api/contractor/availability-admin/${action.payload}`); // Toggles availability for self

        const response = yield axios.get(`/api/contractor/specific/${action.payload}`); // getting updated contractor info
